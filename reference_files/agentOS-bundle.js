      ...
      panel.innerHTML = `
        <div class="aos-header">
          ...
          <div>
            ...
            <button class="aos-btn-close" onclick="AgentOS.toggleAgentPanel()">×</button>
          </div>
        </div>
      ...
      container.innerHTML = Array.from(this.agents.values()).map(agent => `
        <div class="aos-card" data-status="${agent.status}">
          ...
          <div class="aos-card-actions">
            <button onclick="AgentOS.inspect('${agent.id}')">🔍</button>
            <button onclick="AgentOS.focusAgent('${agent.id}')">👁</button>
            <button onclick="AgentOS.destroyAgent('${agent.id}')" class="aos-btn-danger">✕</button>
          </div>
        </div>
      `).join('');
    },
